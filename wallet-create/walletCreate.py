...
with open('walletcreate.json', 'r') as json_file:
    payloads = json.load(json_file)

# Initialize an empty DataFrame to store detailed response data
columns = ['WalletID', 'Status', 'Detail']  # Customize these columns based on the specific data you need
df_responses = pd.DataFrame(columns=columns)

# API endpoint
url = 'https://ig.gov-cloud.ai/mobius-payment-service/v1.0/wallets/create'
...
for payload in payloads:
    ...
    if response.status_code in [200, 201]:
        ...
        row = {
            'WalletID': response_data.get('walletId', 'No ID returned'),
            'Status': 'Success',
...
df_responses.to_excel('walletAPI_response.xlsx', index=False)


# Convert DataFrame to a list of dicts and save to JSON file
response_json_data = df_responses.to_dict(orient='records')
with open('walletAPI_response.json', 'w') as responses_file:
    json.dump(response_json_data, responses_file, indent=4)

print("Responses saved successfully.")

print("Data processing complete. Response saved successfully in JSON and Excel formats.")

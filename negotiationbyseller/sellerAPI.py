...
def approve_negotiation(negotiation_details):
    ...
    url = f"{negotiation_details['base_url']}/{negotiation_details['negotiation_id']}/approve-reject/seller/{negotiation_details['seller_id']}/status/{negotiation_details['status']}"
    response = requests.get(url, headers=headers)
    return response
    
def main():
    # Load negotiation details
    negotiation_details = load_negotiation_details('seller.json')
    all_responses = [] 
